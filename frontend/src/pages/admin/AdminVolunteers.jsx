    ...
    <AdminLayout>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:24 }}>
        ...
        <div style={{ display:'flex', gap:12, alignItems:'center' }}>
          <button style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', border:'1px solid var(--gray-200)', borderRadius:8, fontSize:13, background:'white', color:'var(--gray-600)', cursor:'pointer' }}>
            📍 Filter by Location ▾
          </button>
          ...
          <button style={{ display:'flex', alignItems:'center', gap:6, padding:'9px 16px', background:'var(--amber-400)', color:'white', border:'none', borderRadius:8, fontSize:13, fontWeight:600, cursor:'pointer' }}>
            + Register New
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:16, marginBottom:24 }}>
          {(volunteers.length ? volunteers : MOCK).map((v, i) => (
            <VolunteerCard key={v._id || i} v={v} />
          ))}
          {/* Add new card */}
          <div style={{ border:'2px dashed var(--gray-200)', borderRadius:14, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:32, gap:10, cursor:'pointer', minHeight:200 }}>
  ...
  return (
    ...
    >
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <button style={{ padding:'7px 16px', background:'var(--teal-800)', color:'white', borderRadius:7, fontSize:12, fontWeight:600, cursor:'pointer' }}>View Profile</button>
      </div>
    </div>
  );
}
...
const MOCK = [
  ...
  { fullName:'David Okoro', email:'d.okoro@sentinel.org', emergencyRole:'Support', rating:3.5, district:'South-East Hub', sector:'Zone C', skills:['PB'] },
